...
StatusType CoursesManager::RemoveCourse(const int courseID)
{
    ...
    try
    {
        ...
        std::shared_ptr<Array<int>> course_times=main_tree.getTimesOfCourse(courseID);
        for (int i=0;i<num_of_classes;i++)
        {
            int time_of_class=(*course_times)[i];
            if (time_of_class!=0)
            {
...
StatusType CoursesManager::WatchClass(const int courseID,const int classID,const int time)
{
    ...
    try
    {
         std::shared_ptr<int> time_watched(new int);   
        StatusType state=this->TimeViewed(courseID,classID,time_watched.get());
        if (state!=SUCCESS)
        {
            return state;
        }
