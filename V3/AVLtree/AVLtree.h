...
class Node
    {
        ...
        Node* sucessor;
        Node* predecessor;
        Node():key(U()),value(T()),parent(nullptr),left(nullptr),right(nullptr),sucessor(nullptr),predecessor(nullptr),height(0),bf(0){};
        Node(const U& key,const T& val):key(key),value(val),parent(nullptr),left(nullptr), right(nullptr),predecessor(nullptr),sucessor(nullptr),height(0),bf(0){};
    };
...
class AVLtree 
{
    ...
    void insert(const U& key,const T& val);
     void printInOrder(const Node<U,T>*r);
    void erase(const U& key);
    T* find(const U &key);
    int getTreeSize() const;
    void minKElementsToArray( const int elements_num, Array<T>& values_array,int starting_index);

    void addNode(U a,T b);
    void treeClear();
...
template <class U,class T>
void AVLtree<U,T>::minKElementsToArray( const int elements_num, Array<T>& values_array,int starting_index)
{
    if (elements_num>this->getTreeSize())
    {
        throw;//
    }
    Node<U,T>* curr=min_node;
    for (int i=0;i<elements_num;i++)
    {
        values_array[starting_index+i]=curr->value;
        curr=curr->sucessor;
    }
    return;
...
void AVLtree<U,T>::erase(const U& key)
{
        ...
        if (deleted_node->right!=nullptr && deleted_node->left!=nullptr)
    {
        ...
        {
            deleted_node->predecessor->sucessor=deleted_node;
        }*/
    }
    else
    {
...
template<class U,class T>
class AVLtree<U,T>::iterator
{
    const Node<U,T>* curr_node_ptr;
    friend class AVLtree;
    explicit iterator(const Node<U,T>* r);
    public:
    const Pair<U,T> operator*() const;
    iterator& operator++();
...
template<class U,class T>
AVLtree<U,T>::iterator::iterator(const Node<U,T>* r)
:curr_node_ptr(r){};

template<class U,class T>
const Pair<U,T> AVLtree<U,T>::iterator::operator*() const
{
    Pair<U,T> pair_me(curr_node_ptr->key,curr_node_ptr->value);
    return pair_me;
}

template<class U,class T>
typename AVLtree<U,T>::iterator& AVLtree<U,T>::iterator::operator++()
{
    curr_node_ptr=curr_node_ptr->sucessor;
    return *this;
}

template<class U,class T>
typename AVLtree<U,T>::iterator AVLtree<U,T>::iterator::operator++(int)
{
    AVLtree<U,T>::iterator result=*this;
    ++*this;
    return result;
}

template<class U,class T>
bool AVLtree<U,T>::iterator::operator==(const iterator &it) const
{
    return (curr_node_ptr == it.curr_node_ptr);
}
...
template<class U,class T>
typename AVLtree<U,T>::iterator AVLtree<U,T>::begin()
{
...
template<class U,class T>
typename AVLtree<U,T>::iterator AVLtree<U,T>::end()
{
